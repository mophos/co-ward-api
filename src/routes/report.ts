...
router.get('/get-bed/excel/new', async (req: Request, res: Response) => {
  ...
  try {
    ...
    wsAll.cell(1, 20).string('Cohort ICU ทั้งหมด');
    wsAll.cell(1, 21).string('Cohort ICU ใช้ไปแล้ว');
    wsAll.cell(1, 22).string('Cohort ICU คงเหลือ');
    wsAll.cell(1, 23).string('Hospitel ทั้งหมด');
    wsAll.cell(1, 24).string('Hospitel ใช้ไปแล้ว');
    wsAll.cell(1, 25).string('Hospitel คงเหลือ');
    wsAll.cell(1, 26).string('Hospital Type');
    let rowAll = 2;
    for (const v of zoneCodes) {
      let row = 2;
      ...
      for (const d of data) {
        ...
        ws.cell(1, 20).string('Cohort ICU ทั้งหมด');
        ws.cell(1, 21).string('Cohort ICU ใช้ไปแล้ว');
        ws.cell(1, 22).string('Cohort ICU คงเหลือ');
        ws.cell(1, 23).string('Hospitel ทั้งหมด');
        ws.cell(1, 24).string('Hospitel ใช้ไปแล้ว');
        ws.cell(1, 25).string('Hospitel คงเหลือ');
        ws.cell(1, 26).string('Hospital Type');
        ...
        ws.cell(row, 20).number(d['cohort_icu_covid_qty'] || 0);
        ws.cell(row, 21).number(d['cohort_icu_usage_qty'] || 0);
        ws.cell(row, 22).number(+d['cohort_icu_covid_qty'] - +d['cohort_icu_usage_qty'] || 0);
        ws.cell(row, 23).number(d['hospitel_covid_qty'] || 0);
        ws.cell(row, 24).number(d['hospitel_usage_qty'] || 0);
        ws.cell(row, 25).number(+d['hospitel_covid_qty'] - +d['hospitel_usage_qty'] || 0);
        ws.cell(row, 26).string(d.hospital_type);
        row++
        ...
        wsAll.cell(row, 5).number(d['aiir_covid_qty'] === null ? 0 : d['aiir_covid_qty'] || 0);
        wsAll.cell(row, 6).number(d['aiir_usage_qty'] === null ? 0 : d['aiir_usage_qty'] || 0);
        ...
        wsAll.cell(row, 11).number(d['cohort_icu_covid_qty'] || 0);
        wsAll.cell(row, 12).number(d['cohort_icu_usage_qty'] || 0);
        wsAll.cell(row, 13).number(+d['cohort_icu_covid_qty'] - +d['cohort_icu_usage_qty'] || 0);
        wsAll.cell(row, 14).number(d['isolate_covid_qty'] || 0);
        wsAll.cell(row, 15).number(d['isolate_usage_qty'] || 0);
        wsAll.cell(row, 16).number(+d['isolate_covid_qty'] - +d['isolate_usage_qty'] || 0);
        wsAll.cell(row, 17).number(d['cohort_covid_qty'] || 0);
        wsAll.cell(row, 18).number(d['cohort_usage_qty'] || 0);
        wsAll.cell(row, 19).number(+d['cohort_covid_qty'] - +d['cohort_usage_qty'] || 0);
        wsAll.cell(row, 20).number(d['cohort_icu_covid_qty'] || 0);
        wsAll.cell(row, 21).number(d['cohort_icu_usage_qty'] || 0);
        wsAll.cell(row, 22).number(+d['cohort_icu_covid_qty'] - +d['cohort_icu_usage_qty'] || 0);
        wsAll.cell(row, 23).number(d['hospitel_covid_qty'] || 0);
        wsAll.cell(row, 24).number(d['hospitel_usage_qty'] || 0);
        wsAll.cell(row, 25).number(+d['hospitel_covid_qty'] - +d['hospitel_usage_qty'] || 0);
        wsAll.cell(row, 26).string(d.hospital_type);
        rowAll++;
      }
    }
