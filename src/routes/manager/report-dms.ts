...
router.get('/report6/excel', async (req: Request, res: Response) => {
  ...
  try {
    ...
    ws.cell(1, 21, 2, 21, true).string('Hospital Type');
    ws.cell(1, 22, 2, 22, true).string('ข้อมูลล่าสุด');
    ...
    for (const items of rs) {
      ...
      ws.cell(row++, 22).string(toString(items['entry_date'])).style(right);
    }
...
router.get('/report7/excel', async (req: Request, res: Response) => {
  ...
  try {
    ...
    ws.cell(3, 3).number(toNumber((sumBy(rs, 'non_invasive_qty')) || 0)).style(right);
    ...
    for (const items of rs) {
      ...
      ws.cell(row, 1).string(toString(items['hospname']));
      ws.cell(row, 2).number(toNumber(items['non_invasive_ventilator'])).style(right);
      ws.cell(row, 3).number(toNumber(items['non_invasive_qty'])).style(right);
      ws.cell(row, 4).number(toNumber(items['invasive_ventilator'])).style(right);
      ws.cell(row, 5).number(toNumber(items['invasive_qty'])).style(right);
      ws.cell(row, 6).number(toNumber(items['high_flow'])).style(right);
      ws.cell(row, 7).number(toNumber(items['high_flow_qty'])).style(right);
      ws.cell(row, 8).number(toNumber(items['papr'])).style(right);
      ws.cell(row, 9).number(toNumber(items['papr_qty'])).style(right);
    ...
    ws.cell(row, 3).number(toNumber((sumBy(rs, 'non_invasive_qty')) || 0)).style(right);
